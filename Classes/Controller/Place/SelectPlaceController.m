...
@implementation SelectPlaceController
...
- (UITableViewCell *)tableView:(UITableView *)theTableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    Place* placeObj = [dataList objectAtIndex:row];
    
    [PlaceControllerUtils setCellInfoWithPlace:placeObj cell:cell];
    if ([placeObj.placeId isEqualToString:place.placeId]){
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
	...
	Place* placeObj = [dataList objectAtIndex:indexPath.row];
    self.place = placeObj;

    [self.dataTableView reloadData];    // lazy guy do this...
}
...
{
    SelectPlaceController* vc = [[[SelectPlaceController alloc] init] autorelease];
    vc.place = defaultPlace;
...
{
    if (delegate && [delegate respondsToSelector:@selector(selectPlace:)]){
        [delegate selectPlace:place];
    }
